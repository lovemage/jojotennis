...
export default function InstallPrompt() {
  ...
  if ((!promptEvent && !showIosHint) || !hasScrolled) return null;
  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-40 mx-auto max-w-md rounded-2xl border border-pine/10 bg-white p-4 shadow-[0_18px_48px_rgba(30,61,47,0.16)]">
      ...
      <p className="mt-1 text-xs leading-5 text-muted">
        {promptEvent ? "安裝後可像 App 一樣開啟。" : "請點 Safari 分享按鈕，再選擇「加入主畫面」。"}
      </p>
      <div className="mt-3 flex gap-2">
        {promptEvent ? (
          <button type="button" onClick={() => void install()} className="rounded-lg bg-pine px-4 py-2 text-xs font-bold text-white">
            安裝
          </button>
        ) : null}
        <button
